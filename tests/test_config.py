...
def _isolate(monkeypatch, tmp_path):
    user = tmp_path / "user" / ".wgtl.toml"
    proj = tmp_path / "proj" / ".wgtl.toml"
    monkeypatch.setattr("wgtl_api_cli.config._user_dotfile", lambda: user)
    monkeypatch.setattr("wgtl_api_cli.config._project_dotfile", lambda: proj)
...
def test_cli_flags_beat_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("WAGTAIL_BASE_URL", "https://env.example")
    monkeypatch.setenv("WAGTAIL_TOKEN", "env-tok")
