...
variables, dotfiles, or `wgtl init`.
...
| 2 | Environment variables | `WAGTAIL_BASE_URL`, `WAGTAIL_TOKEN` |
| 3 | Project dotfile | `./.wgtl.toml` |
| 4 (lowest) | User dotfile | `~/.wgtl.toml` |
...
export WAGTAIL_TOKEN="wagtail_abc123def456"
```
...
`wgtl init` writes the user dotfile interactively:
...
The project dotfile lives at `./.wgtl.toml` in the current working directory
