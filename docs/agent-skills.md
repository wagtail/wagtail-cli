...
wagtail-cli ships [agent skills](https://agentskills.io/) that help agents get better results with agentic coding. They are published with the documentation site so they can be discovered and reused with a wide range of tools.

## Editing the skills

Skills live under `src/wagtail_cli/.agents/skills/`. Edit the `SKILL.md` files there; the post-build hook in `docs/hooks.py` publishes them automatically.

## Direct link to the skill

The published skill is served from the documentation site at `.well-known/agent-skills/wagtail-cli/SKILL.md`. Open the URL to read the skill, point an agent at it, or download it and place it in `.agents/skills/wagtail-cli/SKILL.md` in your project so tooling that reads local skills picks it up.
